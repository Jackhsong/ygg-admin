...
<div data-options="region:'center',title:'运费结算'" style="padding:5px;">
	<form id="methodForm" action="" method="post">
		...
		<a id="searchBtn" onclick="doFormAction(2)" href="javascript:;" class="easyui-linkbutton" >确认运费结算</a>&nbsp;&nbsp;&nbsp;
		<a id="searchBtn" onclick="doFormAction(3)" href="javascript:;" class="easyui-linkbutton" >撤销运费结算</a>&nbsp;&nbsp;&nbsp;
	</form>
	<br/>
	<div>
...
function doFormAction(type) {
	$('#type').val(type);
	if(type == 1){
		$('#methodForm').attr("action", "importPostageSettlement").attr("enctype", "multipart/form-data").submit();
	}else if(type == 2){
		var confirmDate = $("#confirmDate").val();
		if(confirmDate == ''){
			$.messager.alert("info","请选择导入日期","error");
		}else{
			$('#methodForm').attr("action", "importPostageSettlement").submit();			
		}
	}else if(type == 3){
		$.messager.confirm("info","你确定要批量撤销运费结算信息吗？",function(r){
			if(r){
				$('#methodForm').attr("action", "importPostageSettlement").submit();				
			}
		});
...
function downFile() {
	window.location.href = '${rc.contextPath}/finance/downloadPostageSettlementTemplate';
}

function downResult() {
	window.location.href = '${rc.contextPath}/finance/downloadImportPostageSettlementResult?dataKey='+$("#dataKey").val();
}
...
$(function (){
	$('#s_data').datagrid({
		...
		collapsible : true,
		idField : 'pid',
		url : '${rc.contextPath}/finance/jsonImportPostageSettlementResult',
		queryParams : {
			dataKey:0
		},
		loadMsg : '正在装载数据...',
		...
		columns : [ [
				...
				{field : 'postage', title : '运费金额', width : 70, align : 'center'},
				] ]
		});
	...
	$('#methodForm').form({
		...
		success:function(data){
			...
			if (data.status == 1){
				...
				if(data.repetitionNum > 0){
                    $.messager.alert("提示","保存成功，去重了"+data.repetitionNum+"条订单。","info");
				}
			}else{
				$('#isRight').val(0);
				$('#okNum').html(0);
