	...
	public String updateLog(int id , int type) throws Exception{
		...
		if(status == 1){
	        ...
	        if(type == 1){
	            ...
	            reqHandler.setParameter("mch_appid", YggAdminProperties.getInstance().getPropertie("qqbs_weixin_appid")); // 公众账号ID
	            reqHandler.setParameter("mchid",YggAdminProperties.getInstance().getPropertie("qqbs_weixin_mchid")); // 商户号
	            //设备号
...
//	            reqHandler.setParameter("re_user_name", "马花花");
	            ...
	            reqHandler.setParameter("spbill_create_ip", "192.168.0.1");
	             ...
	             //key
	            reqHandler.setKey(YggAdminProperties.getInstance().getPropertie("qqbs_weixin_key"));
	            
	            // 生成签名
	            String sign = reqHandler.createSign(reqHandler.getParameters());
	            reqHandler.setParameter("sign", sign);
	            String requestUrl = reqHandler.getRequestURL();
	            String cashUrl = YggAdminProperties.getInstance().getPropertie("qqbs_weixin_cash_url");
	            String mchid = YggAdminProperties.getInstance().getPropertie("qqbs_weixin_mchid");
	            String certPath = YggAdminProperties.getInstance().getPropertie("qqbs_weixin_cash_cert");
