...
{
    private static Logger logger = Logger.getLogger(QqbsBrandController.class);
    ...
    public ModelAndView list(@PathVariable("id") int id)
        throws Exception
    {
        ModelAndView mv = new ModelAndView("qqbsBrand/qqbsBrand");
        this.brandCategoryId = id;
        String brandCategoryName = qqbsBrandService.getBrandCategoryName(id);
    ...
    public String saveOrUpdate(HttpServletRequest request)
    {
        ...
        try
        {
            QqbsBrandEntity brand = new QqbsBrandEntity();
            CommonUtil.wrapParamter2Entity(brand, request);
            if(0==brand.getId()){
                status = qqbsBrandService.addBrand(brand);
            }else{
                status = qqbsBrandService.updateBrand(brand);
            }
            msg = "保存成功";
        }
        catch (Exception e)
            ...
            @RequestParam(value = "categoryId", required = true, defaultValue = "0") int categoryId)
    {
        ...
        try
        {
            ...
            resultMap = qqbsBrandService.findBrandInfo(para);
        }
        catch (Exception e)
        {
    ...
    public String updateBrandDisplay(HttpServletRequest request)
    {
        ...
        try
        {
            QqbsBrandEntity brand = new QqbsBrandEntity();
            CommonUtil.wrapParamter2Entity(brand, request);
            status = qqbsBrandService.updateBrandDisplay(brand);
            msg = "保存成功";
        }
        catch (Exception e)
        {
