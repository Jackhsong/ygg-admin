...
/**
  ...
  * @version $Id: WelfareGroupController.java 9751 2016-04-06 07:56:30Z zhangyibo $   
  * @since 2.0
  */
@Controller
@RequestMapping("/welfareGroup")
public class WelfareGroupController {
	
	
	 /**日志工具    */
	Logger logger = Logger.getLogger(WelfareGroupController.class);
	
	
	 /**格格福利团服务接口    */
	@Resource(name="welfareGroupService")
	private WelfareGroupService welfareGroupService;
	/**
	 * 格格福利团推荐
	 * @return ModelAndView
	 */
	...
	public ModelAndView list() {
		ModelAndView mv = new ModelAndView("welfaregroup/list");
		return mv;
	}
	...
	public Object listInfo(String title,
		...
		try {
			...
			param.put("size", rows);
			return JSON.toJSONString(welfareGroupService.findListInfo(param));
		} catch (Exception e) {
	...
	public Object findById(HttpServletResponse response, int id) {
		try {
			...
			resultMap.put("data", welfareGroupService.findById(id));
	...
	public Object saveOrUpdate(@RequestParam Map<String, Object> param) {
		try {
			Map<String, Object> resultMap = new HashMap<String, Object>();
			resultMap.put("status", 1);
			resultMap.put("data", welfareGroupService.saveOrUpdateInfo(param));
			return resultMap;
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			Map<String, Object> resultMap = new HashMap<String, Object>();
        ...
        throws Exception
    {
        ...
        int resultStatus = welfareGroupService.delete(id);
        if (resultStatus != 1){
            resultMap.put("status", 0);
